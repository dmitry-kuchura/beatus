...
/*
 Ð±Ñ‹ÑÑ‚Ñ€Ñ‹Ð¹ Ð¿Ð¾Ð¸ÑÐº Ð¿Ð¾ Ð²Ñ‹Ð´ÐµÐ»ÐµÐ½Ð¸ÑŽ
 WezomFix - Ñ„Ð¸ÐºÑÑ‹ ÑÐºÑ€Ð¸Ð¿Ñ‚Ð°
 WezomRules - Ð¿Ñ€Ð°Ð²Ð¸Ð»Ð° Ð²Ð°Ð»Ð¸Ð´Ð°Ñ†Ð¸Ð¸
 */
...
(function (factory) {
    ...
    $.extend($.fn, {
        ...
        wFileVal: function (input, isLabel) {
            ...
            var noSelected = 'ÐÐ¸Ñ‡ÐµÐ³Ð¾ Ð½Ðµ Ð²Ñ‹Ð±Ñ€Ð°Ð½Ð¾';
            var isSelected = (multiple) ? 'Ð’Ñ‹Ð±Ñ€Ð°Ð½Ð¾ Ñ„Ð°Ð¹Ð»Ð¾Ð² - %count: <ins>(%files)</ins>' : 'Ð’Ñ‹Ð±Ñ€Ð°Ð½Ð¾: <ins>%files</ins>';
    ...
    $.extend($.validator, {
        ...
        methods: {
            ...
            word: function (value, element) {
                return this.optional(element) || /^[a-zA-ZÐ°-ÑÐ-Ð¯Ñ–Ð†Ñ—Ð‡Ñ”Ñ‘ÐÐ„Ò‘Ò\'\`\- ]*$/.test(value);
            },

            login: function (value, element) {
                return this.optional(element) || /^[0-9a-zA-ZÐ°-ÑÐ-Ð¯Ñ–Ð†Ñ—Ð‡Ñ”Ð„Ñ‘ÐÒ‘Ò][0-9a-zA-ZÐ°-ÑÐ-Ð¯Ñ–Ð†Ñ—Ð‡Ñ”Ð„Ò‘Ò\-\._]+$/.test(value);
            },

            phoneUA: function (value, element, param) {
                return this.optional(element) || /^(((\+?)(\d{1,3}))\s?)?(([0-9]{2,3})|(\([0-9]{3}\)))(\-|\s)?(([0-9]{3})(\-|\s)?([0-9]{2})(\-|\s)?([0-9]{2})|([0-9]{2})(\-|\s)?([0-9]{2})(\-|\s)?([0-9]{3})|([0-9]{2})(\-|\s)?([0-9]{3})(\-|\s)?([0-9]{2}))$/.test(value);
            },
...
window.wLang = (function (window, document, undefined) {
    // Ð¼Ð°ÑÑÐ¸Ð² Ð¸Ð¼ÐµÑŽÑ‰Ð¸Ñ…ÑÑ Ð»Ð¾ÐºÐ°Ð»Ð¸Ð·Ð°Ñ†Ð¸Ð¸
    var names = ['ru', 'ua', 'pl', 'en'];
    // Ñ„Ð»Ð°Ð³ Ð¾Ñ‚ÑÑƒÑ‚ÑÑƒÑ‚Ð²ÑƒÑŽÑ‰ÐµÐ¹ Ð»Ð¾ÐºÐ°Ð»Ð¸Ð·Ð°Ñ†Ð¸Ð¸
    var miss = true;
    // ÑÐ·Ñ‹Ðº Ð¿Ð¾ ÑƒÐ¼Ð¾Ð»Ñ‡Ð°Ð½Ð¸ÑŽ
    var def = 'ru';
    // Ð½Ð°Ð²Ð¸Ð³Ð°Ñ‚Ð¾Ñ€ Ð¾Ð±ÑŠÐµÐºÑ‚Ð° Window
    var navi = window.navigator;
    // Ð¾Ð¿Ñ€ÐµÐ´ÐµÐ»ÑÐµÐ¼ ÑÐ·Ñ‹Ðº Ð´Ð¾ÐºÑƒÐ¼ÐµÐ½Ñ‚Ð°
    var lang = document.documentElement.getAttribute('lang') ||
        navi.language ||
        navi.browserLanguage ||
        w.userLanguage || def;

    // Ð¾Ñ‚ ÐºÐ¾Ð´Ð° ÑÐ·Ñ‹ÐºÐ° Ð¾Ñ‚ÑÐµÐºÐ°ÐµÐ¼ ÐºÐ¾Ð´ Ñ€ÐµÐ³Ð¸Ð¾Ð½Ð°(ÐµÑÐ»Ð¸ Ð¿Ñ€Ð¸ÑÑƒÑ‚ÑÑ‚Ð²ÑƒÐµÑ‚) Ð¸ Ð¿ÐµÑ€ÐµÐ²Ð¾Ð´Ð¸Ð¼ Ð² Ð½Ð¸Ð¶Ð½Ð¸Ð¹ Ñ€ÐµÐ³Ð¸ÑÑ‚Ñ€
    lang = lang.toLowerCase().substr(0, 2);

    // ÑÐ²ÐµÑ€ÑÐµÐ¼ Ð¿Ð¾Ð»ÑƒÑ‡ÐµÐ½Ñ‹Ð¹ Ñ€ÐµÐ·ÑƒÐ»ÑŒÑ‚Ð°Ñ‚ Ñ Ð¸Ð¼ÐµÑŽÑ‰Ð¸Ð¼ÑÑ Ð¼Ð°ÑÑÐ¸Ð²Ð¾Ð¼
    ...
    // ÐµÑÐ»Ð¸ Ð¿Ð¾Ð»ÑƒÑ‡Ð°ÐµÐ¼ Ð¾Ñ‚ÑÑƒÑ‚ÑÑ‚Ð²ÑƒÑŽÑ‰Ð¸Ð¹ Ð²Ð°Ñ€Ð¸Ð°Ð½Ñ‚ Ð¾Ð¿Ñ€ÐµÐ´ÐµÐ»ÑÐµÐ¼ ÑÐ·Ñ‹Ðº Ð¿Ð¾ ÑƒÐ¼Ð¾Ð»Ñ‡Ð°Ð½Ð¸ÑŽ
    if (miss) {
        lang = def;
    }

    // Ð²Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°ÐµÑ‚ ÑÐ·Ñ‹Ðº Ð² ÐºÐ°Ñ‡ÐµÑÑ‚Ð²Ðµ Ð½Ð¾Ð²Ð¾Ð³Ð¾ ÑÐ²Ð¾Ð¹Ñ‚ÑÑ‚Ð²Ð° Ð¾Ð±ÑŠÐµÐºÑ‚Ñƒ Window
    // Ð´Ð°Ð»ÐµÐµ ÑÐ²Ð¾Ð¹Ñ‚Ð²Ð¾ Ð¼Ð¾Ð¶Ð½Ð¾ Ð¸ÑÐ¿Ð¾Ð»ÑŒÐ·Ð°Ð²Ð°Ñ‚ÑŒ ÐºÐ°Ðº Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ Ð³Ð»Ð¾Ð±Ð°Ð»ÑŒÐ½Ð¾Ð¹ Ð¿ÐµÑ€ÐµÐ¼ÐµÐ½Ð½Ð¾Ð¹ wLang
    // console.log(wLang);
    return lang;
...
window.wLocalize = (function (window, document, undefined) {
    // Ð¾Ð±ÑŠÐµÐºÑ‚ ÑÐ¾Ð´ÐµÑ€Ð¶Ð°Ñ‰Ð¸Ð¹ Ð¿ÐµÑ€ÐµÐ²Ð¾Ð´Ñ‹
    var localize = {};

    // Ð¿ÐµÑ€ÐµÐ²Ð¾Ð´Ñ‹ Ð´Ð»Ñ Ð¿Ð»Ð°Ð³Ð¸Ð½Ð° jquery.magnific-popup
    localize.Magnific = {
        ru: {
            tClose: 'Ð—Ð°ÐºÑ€Ñ‹Ñ‚ÑŒ (ESC)',
            tLoading: 'Ð—Ð°Ð³Ñ€ÑƒÐ·ÐºÐ° ÐºÐ¾Ð½Ñ‚ÐµÐ½Ñ‚Ð° ...',
            tNotFound: 'ÐšÐ¾Ð½Ñ‚ÐµÐ½Ñ‚ Ð½Ðµ Ð½Ð°Ð¹Ð´ÐµÐ½',
            tError: 'ÐÐµÐ²Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ð·Ð°Ð³Ñ€ÑƒÐ·Ð¸Ñ‚ÑŒ <a href="%url%" target="_blank">ÐšÐ¾Ð½Ñ‚ÐµÐ½Ñ‚</a>.',
            tErrorImage: 'ÐÐµÐ²Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ð·Ð°Ð³Ñ€ÑƒÐ·Ð¸Ñ‚ÑŒ <a href="%url%" target="_blank">Ð˜Ð·Ð¾Ð±Ñ€Ð°Ð¶ÐµÐ½Ð¸Ðµ #%curr%</a>.',
            tPrev: 'ÐŸÑ€ÐµÐ´Ñ‹Ð´ÑƒÑ‰Ð°Ñ (ÐºÐ»Ð°Ð²Ð¸ÑˆÐ° Left)',
            tNext: 'Ð¡Ð»ÐµÐ´ÑƒÑŽÑ‰Ð°Ñ (ÐºÐ»Ð°Ð²Ð¸ÑˆÐ° Right)',
            tCounter: '%curr% Ð¸Ð· %total%'
        },
        ua: {
            tClose: 'Ð—Ð°ÐºÑ€Ð¸Ñ‚Ð¸ (ESC)',
            tLoading: 'Ð—Ð°Ð²Ð°Ð½Ñ‚Ð°Ð¶ÐµÐ½Ð½Ñ ÐºÐ¾Ð½Ñ‚ÐµÐ½Ñ‚Ñƒ ...',
            tNotFound: 'ÐšÐ¾Ð½Ñ‚ÐµÐ½Ñ‚ Ð½Ðµ Ð·Ð½Ð°Ð¹Ð´ÐµÐ½Ð¾',
            tError: 'ÐÐµÐ¼Ð¾Ð¶Ð»Ð¸Ð²Ð¾ Ð·Ð°Ð²Ð°Ð½Ñ‚Ð°Ð¶Ð¸Ñ‚Ð¸ <a href="%url%" target="_blank">ÐºÐ¾Ð½Ñ‚ÐµÐ½Ñ‚</a>.',
            tErrorImage: 'ÐÐµÐ¼Ð¾Ð¶Ð»Ð¸Ð²Ð¾ Ð·Ð°Ð²Ð°Ð½Ñ‚Ð°Ð¶Ð¸Ñ‚Ð¸ <a href="%url%" target="_blank">Ð—Ð¾Ð±Ñ€Ð°Ð¶ÐµÐ½Ð½Ð½Ñ #%curr%</a>.',
            tPrev: 'ÐŸÐ¾Ð¿ÐµÑ€ÐµÐ´Ð½Ñ (ÐºÐ»Ð°Ð²Ñ–ÑˆÐ° Left)',
            tNext: 'ÐÐ°ÑÑ‚ÑƒÐ¿Ð½Ð° (ÐºÐ»Ð°Ð²Ñ–ÑˆÐ° Right)',
            tCounter: '%curr% Ð· %total%'
        },
        pl: {
            tClose: 'ZamknÄ…Ä‡ (ESC)',
            tLoading: 'ZawartoÅ›Ä‡ do treÅ›ci ...',
            tNotFound: 'Nie znaleziono treÅ›ci',
            tError: 'NiemoÅ¼liwe do Å›ciÄ…gniÄ™cia <a href="%url%" target="_blank">treÅ›ci</a>.',
            tErrorImage: 'NiemoÅ¼liwe do Å›ciÄ…gniÄ™cia <a href="%url%" target="_blank">ObrazÃ³w #%curr%</a>.',
            tPrev: 'Poprzednia (klucz Left)',
            tNext: 'NastÄ™pna (klucz Right)',
    ...
    // Ð¿ÐµÑ€ÐµÐ²Ð¾Ð´Ñ‹ Ð´Ð»Ñ Ð¿Ð»Ð°Ð³Ð¸Ð½Ð° jquery-validation
    localize.Validate = {
        ru: {
            required: "Ð­Ñ‚Ð¾ Ð¿Ð¾Ð»Ðµ Ð½ÐµÐ¾Ð±Ñ…Ð¾Ð´Ð¸Ð¼Ð¾ Ð·Ð°Ð¿Ð¾Ð»Ð½Ð¸Ñ‚ÑŒ!",
            required_checker: "Ð­Ñ‚Ð¾Ñ‚ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ - Ð¾Ð±ÑÐ·Ð°Ñ‚ÐµÐ»ÐµÐ½!",
            required_select: "Ð­Ñ‚Ð¾Ñ‚ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ - Ð¾Ð±ÑÐ·Ð°Ñ‚ÐµÐ»ÐµÐ½!",
            required: "Ð­Ñ‚Ð¾ Ð¿Ð¾Ð»Ðµ Ð½ÐµÐ¾Ð±Ñ…Ð¾Ð´Ð¸Ð¼Ð¾ Ð·Ð°Ð¿Ð¾Ð»Ð½Ð¸Ñ‚ÑŒ!",
            password: "Ð£ÐºÐ°Ð¶Ð¸Ñ‚Ðµ ÐºÐ¾Ñ€Ñ€ÐµÐºÑ‚Ð½Ñ‹Ð¹ Ð¿Ð°Ñ€Ð¾Ð»ÑŒ!",
            ...
            maxlength: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ð²ÐµÐ´Ð¸Ñ‚Ðµ Ð½Ðµ Ð±Ð¾Ð»ÐµÐµ {0} ÑÐ¸Ð¼Ð²Ð¾Ð»Ð¾Ð²!",
            maxlength_checker: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð½Ðµ Ð±Ð¾Ð»ÐµÐµ {0} Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ð¾Ð²!",
            maxlength_select: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð½Ðµ Ð±Ð¾Ð»ÐµÐµ {0} Ð¿ÑƒÐ½ÐºÑ‚Ð¾Ð²!",
            minlength: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ð²ÐµÐ´Ð¸Ñ‚Ðµ Ð½Ðµ Ð¼ÐµÐ½ÐµÐµ {0} ÑÐ¸Ð¼Ð²Ð¾Ð»Ð¾Ð²!",
            minlength_checker: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð½Ðµ Ð¼ÐµÐ½ÐµÐµ {0} Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ð¾Ð²!",
            minlength_select: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð½Ðµ Ð¼ÐµÐ½ÐµÐµ {0} Ð¿ÑƒÐ½ÐºÑ‚Ð¾Ð²!",
            rangelength: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ð²ÐµÐ´Ð¸Ñ‚Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ Ð´Ð»Ð¸Ð½Ð¾Ð¹ Ð¾Ñ‚ {0} Ð´Ð¾ {1} ÑÐ¸Ð¼Ð²Ð¾Ð»Ð¾Ð²!",
            rangelength_checker: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð¾Ñ‚ {0} Ð´Ð¾ {1} Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ð¾Ð²!",
            rangelength_select: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, Ð²Ñ‹Ð±ÐµÑ€Ð¸Ñ‚Ðµ Ð¾Ñ‚ {0} Ð´Ð¾ {1} Ð¿ÑƒÐ½ÐºÑ‚Ð¾Ð²!",
            range: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, ÑƒÐºÐ°Ð¶Ð¸Ñ‚Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ Ð¾Ñ‚ {0} Ð´Ð¾ {1}!",
            filetype: "Ð”Ð¾Ð¿ÑƒÑÑ‚Ð¸Ð¼Ñ‹Ðµ Ñ€Ð°ÑÑˆÐ¸Ñ€ÐµÐ½Ð¸Ñ Ñ„Ð°Ð¹Ð»Ð¾Ð²: {0}!",
            filesize: "ÐœÐ°ÐºÑÐ¸Ð¼Ð°Ð»ÑŒÐ½Ñ‹Ð¹ Ð¾Ð±ÑŠÐµÐ¼ {0}KB!",
            filesizeEach: "ÐœÐ°ÐºÑÐ¸Ð¼Ð°Ð»ÑŒÐ½Ñ‹Ð¹ Ð¾Ð±ÑŠÐµÐ¼ ÐºÐ°Ð¶Ð´Ð¾Ð³Ð¾ Ñ„Ð°Ð¹Ð»Ð° {0}KB!",
            max: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, ÑƒÐºÐ°Ð¶Ð¸Ñ‚Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ, Ð¼ÐµÐ½ÑŒÑˆÐµ Ð¸Ð»Ð¸ Ñ€Ð°Ð²Ð½Ð¾ÐµÂ {0}!",
            min: "ÐŸÐ¾Ð¶Ð°Ð»ÑƒÐ¹ÑÑ‚Ð°, ÑƒÐºÐ°Ð¶Ð¸Ñ‚Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ, Ð±Ð¾Ð»ÑŒÑˆÐµ Ð¸Ð»Ð¸ Ñ€Ð°Ð²Ð½Ð¾Ðµ {0}!",
            pattern: "Ð£ÐºÐ°Ð¶Ð¸Ñ‚Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ ÑÐ¾Ð¾Ñ‚Ð²ÐµÑ‚ÑÑ‚Ð²ÑƒÑŽÑ‰ÐµÐµ Ð¼Ð°ÑÐºÐµ {0}!",
            // add
            word: "Ð’Ð²ÐµÐ´Ð¸Ñ‚Ðµ ÐºÐ¾Ñ€Ñ€ÐµÐºÑ‚Ð½Ð¾Ðµ ÑÐ»Ð¾Ð²ÐµÑÐ½Ð¾Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ!",
            login: "Ð’Ð²ÐµÐ´Ð¸Ñ‚Ðµ ÐºÐ¾Ñ€Ñ€ÐµÐºÑ‚Ð½Ñ‹Ð¹ Ð»Ð¾Ð³Ð¸Ð½!",
            phoneUA: "ÐÐµÐºÐ¾Ñ€Ñ€ÐµÐºÑ‚Ð½Ñ‹Ð¹ Ñ„Ð¾Ñ€Ð¼Ð°Ñ‚ ÑƒÐºÑ€Ð°Ð¸Ð½ÑÐºÐ¾Ð³Ð¾ Ð½Ð¾Ð¼ÐµÑ€Ð°"
        },
        ua: {
            required: "Ð¦Ðµ Ð¿Ð¾Ð»Ðµ Ð½ÐµÐ¾Ð±Ñ…Ñ–Ð´Ð½Ð¾ Ð·Ð°Ð¿Ð¾Ð²Ð½Ð¸Ñ‚Ð¸!",
            required_checker: "Ð¦ÐµÐ¹ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ - Ð¾Ð±Ð¾Ð²'ÑÐ·ÐºÐ¾Ð²Ð¸Ð¹!",
            required_select: "Ð¦ÐµÐ¹ Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€ - Ð¾Ð±Ð¾Ð²'ÑÐ·ÐºÐ¾Ð²Ð¸Ð¹!",
            password: "Ð’ÐºÐ°Ð¶Ñ–Ñ‚ÑŒ ÐºÐ¾Ñ€Ñ€ÐµÐºÑ‚Ð½Ð¸Ð¹ Ð¿Ð°Ñ€Ð¾Ð»ÑŒ!",
            ...
            digits: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð²Ð¾Ð´Ð¸Ñ‚Ðµ Ñ‚Ñ–Ð»ÑŒÐºÐ¸ Ñ†Ð¸Ñ„Ñ€Ð¸!",
            creditcard: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð²ÐµÐ´Ñ–Ñ‚ÑŒ Ð¿Ñ€Ð°Ð²Ð¸Ð»ÑŒÐ½Ð¸Ð¹ Ð½Ð¾Ð¼ÐµÑ€ ÐºÑ€ÐµÐ´Ð¸Ñ‚Ð½Ð¾Ñ— ÐºÐ°Ñ€Ñ‚ÐºÐ¸!",
            equalTo: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð²ÐµÐ´Ñ–Ñ‚ÑŒ Ñ‚Ð°ÐºÐµ Ð¶ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ Ñ‰Ðµ Ñ€Ð°Ð·!",
            maxlength: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð²ÐµÐ´Ñ–Ñ‚ÑŒ Ð½Ðµ Ð±Ñ–Ð»ÑŒÑˆ {0} ÑÐ¸Ð¼Ð²Ð¾Ð»Ñ–Ð²!",
            maxlength_checker: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð¸Ð±ÐµÑ€Ñ–Ñ‚ÑŒ Ð½Ðµ Ð±Ñ–Ð»ÑŒÑˆ {0} Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ñ–Ð²!",
            maxlength_select: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð¸Ð±ÐµÑ€Ñ–Ñ‚ÑŒ Ð½Ðµ Ð±Ñ–Ð»ÑŒÑˆ {0} Ð¿ÑƒÐ½ÐºÑ‚Ñ–Ð²!",
            minlength: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð²ÐµÐ´Ñ–Ñ‚ÑŒ Ð½Ðµ Ð¼ÐµÐ½Ñˆ {0} ÑÐ¸Ð¼Ð²Ð¾Ð»Ñ–Ð²!",
            minlength_checker: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð¸Ð±ÐµÑ€Ñ–Ñ‚ÑŒ Ð½Ðµ Ð¼ÐµÐ½ÑˆÐµ {0} Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ñ–Ð²!",
            minlength_select: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð¸Ð±ÐµÑ€Ñ–Ñ‚ÑŒ Ð½Ðµ Ð¼ÐµÐ½Ñˆ {0} Ð¿ÑƒÐ½ÐºÑ‚Ñ–Ð²!",
            rangelength: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ° , Ð²Ð²ÐµÐ´Ñ–Ñ‚ÑŒ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ Ð´Ð¾Ð²Ð¶Ð¸Ð½Ð¾ÑŽ Ð²Ñ–Ð´ {0} Ð´Ð¾ {1} ÑÐ¸Ð¼Ð²Ð¾Ð»Ñ–Ð²!",
            rangelength_checker: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð¸Ð±ÐµÑ€Ñ–Ñ‚ÑŒ Ð²Ñ–Ð´ {0} Ð´Ð¾ {1} Ð¿Ð°Ñ€Ð°Ð¼ÐµÑ‚Ñ€Ñ–Ð²!",
            rangelength_select: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²Ð¸Ð±ÐµÑ€Ñ–Ñ‚ÑŒ Ð²Ñ–Ð´ {0} Ð´Ð¾ {1} Ð¿ÑƒÐ½ÐºÑ‚Ñ–Ð²!",
            range: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²ÐºÐ°Ð¶Ñ–Ñ‚ÑŒ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ Ð²Ñ–Ð´ {0} Ð´Ð¾ {1}!",
            filetype: "Ð”Ð¾Ð¿ÑƒÑÑ‚Ð¸Ð¼Ñ– Ñ€Ð¾Ð·ÑˆÐ¸Ñ€ÐµÐ½Ð½Ñ Ñ„Ð°Ð¹Ð»Ñ–Ð²: {0}!",
            filesize: "ÐœÐ°ÐºÑÐ¸Ð¼Ð°Ð»ÑŒÐ½Ð¸Ð¹ Ð¾Ð±ÑÑÐ³ {0}KB!",
            filesizeEach: "ÐœÐ°ÐºÑÐ¸Ð¼Ð°Ð»ÑŒÐ½Ñ‹Ð¹ Ñ€Ð°Ð·Ð¼ÐµÑ€ ÐºÐ°Ð¶Ð´Ð¾Ð³Ð¾ Ñ„Ð°Ð¹Ð»Ð° {0}KB!",
            max: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²ÐºÐ°Ð¶Ñ–Ñ‚ÑŒ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ, Ð¼ÐµÐ½ÑˆÐµ Ð°Ð±Ð¾ Ñ€Ñ–Ð²Ð½Ðµ {0}!",
            min: "Ð‘ÑƒÐ´ÑŒ Ð»Ð°ÑÐºÐ°, Ð²ÐºÐ°Ð¶Ñ–Ñ‚ÑŒ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ, Ð±Ñ–Ð»ÑŒÑˆÐµ Ð°Ð±Ð¾ Ñ€Ñ–Ð²Ð½Ðµ {0}!.",
            pattern: "Ð’ÐºÐ°Ð¶Ñ–Ñ‚ÑŒ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ Ð²Ñ–Ð´Ð¿Ð¾Ð²Ñ–Ð´Ð½Ðµ {0}!",
            // add
            word: "Ð’Ð²ÐµÐ´Ñ–Ñ‚ÑŒ ÐºÐ¾Ñ€ÐµÐºÑ‚Ð½Ðµ Ñ–Ð¼'Ñ!",
            login: "Ð’Ð²ÐµÐ´Ñ–Ñ‚ÑŒ ÐºÐ¾Ñ€ÐµÐºÑ‚Ð½Ð¸Ð¹ Ð»Ð¾Ð³Ñ–Ð½!",
            phoneUA: "ÐÐµÐºÐ¾Ñ€ÐµÐºÑ‚Ð½Ð¸Ð¹ Ñ„Ð¾Ñ€Ð¼Ð°Ñ‚ ÑƒÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ¾Ð³Ð¾ Ð½Ð¾Ð¼ÐµÑ€Ð°"
        },
        pl: {
            required: "To pole naleÅ¼y wypeÅ‚niÄ‡!",
            required_checker: "Parametr ten jest wymagany!",
            required_select: "Parametr ten jest wymagany!",
            password: "OkreÅ›l paroll!",
            remote: "ProszÄ™, wprowadÅºcie prawidÅ‚owe znaczenie!",
            email: "ProszÄ™ wpisaÄ‡ poprawny adres e-mail!",
            url: "ProszÄ™ podaÄ‡ poprawny adres URL!",
            date: "Wpisz poprawnÄ… datÄ™!",
            dateISO: "ProszÄ™ podaÄ‡ poprawnÄ… datÄ™ w formacie ISO!",
            number: "ProszÄ™ wpisaÄ‡ numery!",
            digits: "ProszÄ™ wpisaÄ‡ tylko liczby!",
            creditcard: "ProszÄ™ podaÄ‡ poprawny numer karty kredytowej!",
            equalTo: "ProszÄ™ ponownie wprowadziÄ‡ wartoÅ›Ä‡!",
            maxlength: "ProszÄ™ wpisaÄ‡ nie wiÄ™cej niÅ¼ {0} znakÃ³w!",
            maxlength_checker: "ProszÄ™ wybraÄ‡ nie wiÄ™cej niÅ¼ {0} parametrÃ³w!",
            maxlength_select: "ProszÄ™ wybraÄ‡ nie wiÄ™cej niÅ¼ {0} pozycji!",
            minlength: "ProszÄ™ podaÄ‡ co najmniej {0} znakÃ³w!",
            minlength_checker: "Wybierz co najmniej {0} opcje!",
            minlength_select: "Wybierz co najmniej {0} przedmioty!",
            rangelength: "Prosimy podaÄ‡ wartoÅ›Ä‡ pomiÄ™dzy {0} {1} znakÃ³w!",
            rangelength_checker: "ProszÄ™ wybraÄ‡ od {0} do {1} parametrÃ³w!",
            rangelength_select: "ProszÄ™ wybraÄ‡ od {0} do {1} punktÃ³w!",
            range: "Prosimy podaÄ‡ wartoÅ›Ä‡ miÄ™dzy {0} - {1}!",
            filetype: "Dopuszczalne rozszerzenia plikÃ³w: {0}!",
            filesize: "Maksymalny rozmiar {0}KB!",
            filesizeEach: "Maksymalny rozmiar kaÅ¼dego plika {0}KB!",
            max: "Prosimy podaÄ‡ wartoÅ›Ä‡ mniejsza lub rÃ³wna {0}!",
            min: "Prosimy podaÄ‡ wartoÅ›Ä‡ wiÄ™kszÄ… lub rÃ³wnÄ… {0}!",
            pattern: "OkreÅ›l wartoÅ›Ä‡ odpowiadajÄ…cÄ… maski {0}!",
            // add
            word: "WprowadÅº poprawne znaczenie sÅ‚Ã³w!",
            login: "ProszÄ™ podaÄ‡ poprawnÄ… nazwÄ™ uÅ¼ytkownika!",
            phoneUA: "NieprawidÅ‚owy format numeru telefonu ukraiÅ„ski"
        },
        en: {
            required: "This field is required!",
    ...
    // Ð²Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°ÐµÑ‚ Ð¾Ð±ÑŠÐµÐºÑ‚ Ñ Ð¿ÐµÑ€ÐµÐ²Ð¾Ð´Ð°Ð¼ Ð² ÐºÐ°Ñ‡ÐµÑÑ‚Ð²Ðµ Ð½Ð¾Ð²Ð¾Ð³Ð¾ ÑÐ²Ð¾Ð¹Ñ‚ÑÑ‚Ð²Ð° Ð¾Ð±ÑŠÐµÐºÑ‚Ñƒ Window
    // Ð´Ð°Ð»ÐµÐµ Ð¾Ð±ÑŠÐµÐºÑ‚ Ð¼Ð¾Ð¶Ð½Ð¾ Ð¸ÑÐ¿Ð¾Ð»ÑŒÐ·Ð°Ð²Ð°Ñ‚ÑŒ ÐºÐ°Ðº Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ Ð³Ð»Ð¾Ð±Ð°Ð»ÑŒÐ½Ð¾Ð¹ Ð¿ÐµÑ€ÐµÐ¼ÐµÐ½Ð½Ð¾Ð¹ wLocalize
    // console.log(wLocalize);

    return localize;

    // Ð¿Ñ€Ð¸Ð¼ÐµÑ€ Ð¸ÑÐ¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ð½Ð¸Ñ
    // Ð¿ÐµÑ€ÐµÐ²ÐµÐ´ÐµÐ½Ð½Ñ‹Ð¹ Ñ‚ÐµÐºÑÑ‚ Ð·Ð°Ð³Ñ€ÑƒÐ·ÐºÐ¸ Ð¿Ð»Ð°Ð³Ð¸Ð½Ð° MagnificPopup
